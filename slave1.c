...
static int slave1_notifier_fn(struct notifier_block *nb, unsigned long action, void *data)
{
...
static int __init slave1_init(void)
{
	int ret;
	printk("###%s %d###\n",__func__,__LINE__);

	my_nb.notifier_call = slave1_notifier_fn;
	my_nb.priority =10;
...
static void __exit slave1_exit(void)
{
...
module_init(slave1_init);
module_exit(slave1_exit);
MODULE_LICENSE("GPL");
